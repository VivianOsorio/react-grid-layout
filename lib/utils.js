...
export function childrenEqual(a: ReactChildren, b: ReactChildren): boolean {
  return (
    deepEqual(
      ...
      React.Children.map(a, c => (c ? c.key : undefined)),
      React.Children.map(b, c => (c ? c.key : undefined))
    ) &&
    deepEqual(
      React.Children.map(a, c => c && c.props && c.props["data-grid"]),
      React.Children.map(b, c => c && c.props && c.props["data-grid"])
    )
